     ...
     (list
      ...
      #~(modify-phases %standard-phases
          ...
          (add-after 'patch-source-shebangs 'replace-prefix
            (lambda* (#:key outputs #:allow-other-keys)
              ;; Use #$output instead of hardcoded /run/current-system/profile
              (substitute* "src/cli/libnvc.c"
                (("libnvidia-ml.so.1") (string-append #$output "/lib/libnvidia-ml.so.1")))
              ...
              (setenv "LDFLAGS" (string-append (or (getenv "LDFLAGS") "") " -ltirpc -lseccomp -lcap -Wl,-rpath=" #$output "/lib"))
